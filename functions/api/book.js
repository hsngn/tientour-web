...
export async function onRequestPost({ request, env }) {
  ...
  const row = {
    ...
    departure_city: b.bus_out || null,
    return_city: b.bus_return || null,
    back_pax: Number(b.back_pax) || 0,
    country: b.country || null,
    accommodation: b.accommodation === 'dorm' ? 'dorm' : 'private',
  ...
  if (env.RESEND_API_KEY && env.NOTIFY_EMAIL) {
    ...
    const html = `
      ...
      <p><b>Accommodation:</b> ${b.accommodation === 'dorm' ? `Dorm beds \u00d7${esc(b.travelers)}` : `Private room \u00d7${esc(b.rooms || 1)}`}</p>
