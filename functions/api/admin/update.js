...
export async function onRequestPost({ request, env }) {
  ...
  const patch = {};
  for (const f of TEXT) if (body[f] !== undefined) patch[f] = body[f] === '' ? null : body[f];
  for (const f of NUM)  if (body[f] !== undefined) patch[f] = (body[f] === '' || body[f] === null) ? null : Number(body[f]);
